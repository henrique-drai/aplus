...
class ScriptModel extends CI_Model {
  ...
  public function admin($name, $surname, $email, $password, $description) {
    $this->db->delete("user", ['email' => $email]);
    $this->db->insert("user", Array("name"=>$name, "surname"=>$surname, "email"=>$email, "role"=>"admin", "password"=>md5($password), "description"=>$description));
  }
  ...
  public function horario_duvidas($id_cadeira, $id_prof, $start_time, $end_time, $day) {
    $this->db->insert("horario_duvidas", Array("id_cadeira"=> $id_cadeira, "id_prof"=>$id_prof, "start_time"=>$start_time, "end_time"=>$end_time, "day"=>$day));
    return $this->db->insert_id();
  }
